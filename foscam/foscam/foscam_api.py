...
class FoscamCamera(object):
    ...
    def send_command(self, cmd, params=None, raw=False, timeout=30):
        '''
        ...
        #with open("foscam_output", 'a+') as file:
        #    file.write("\n" + cmdurl + "\nTimeout: " + str(timeout))
        ...
        try:
            raw_string = ''
            if self.ssl and ssl_enabled:
                gcontext = ssl.SSLContext(ssl.PROTOCOL_TLSv1_2)  # disable cert
                #raw_string = requests.get(cmdurl, timeout=timeout).text
                #print(raw_string)
                raw_string = urlopen(cmdurl,context=gcontext, timeout=timeout).read()
            else:
                raw_string = urlopen(cmdurl,timeout=timeout).read()
            if raw:
                if self.verbose:
            ...
            #with open("/home/homeassistant/foscam_output", 'a+') as file:
            #    file.write("\n" + "!!Exception: " + str(e))
    ...
    def execute_command(self, cmd, params=None, callback=None, raw=False, timeout=30):
        '''
        Execute a command and return a parsed response.
        '''
        def execute_with_callbacks(cmd, params=None, callback=None, raw=False):
            code, params = self.send_command(cmd, params, raw, timeout=timeout)
            if callback:
    ...
    def sleep(self, callback=None, timeout=30):
        '''
        Put camera to sleep.
        '''
        return self.execute_command('alexaSleep', callback=callback, timeout=timeout)

    def wake(self, callback=None, timeout=30):
        '''
        Wake camera from sleep.
        '''
        return self.execute_command('alexaWakeUp', callback=callback, timeout=timeout)

    def is_asleep(self, callback=None, timeout=30):
        '''
        Wakup camera
        '''
        ret, data = self.execute_command('getAlexaState', callback=callback, timeout=timeout)
    ...
    def open_infra_led(self, callback=None, timeout=30):
        '''
        Force open infra led
        cmd: openInfraLed
        '''
        return self.execute_command('openInfraLed', callback=callback, timeout=timeout)

    def close_infra_led(self, callback=None, timeout=30):
        '''
        Force close infra led
        cmd: closeInfraLed
        '''
        return self.execute_command('closeInfraLed', callback=callback, timeout=timeout)

    def get_infra_led_config(self, callback=None, timeout=30):
        '''
        Get Infrared LED configuration
        cmd: getInfraLedConfig
        '''
        return self.execute_command('getInfraLedConfig', callback=callback, timeout=timeout)

    def set_infra_led_config(self, mode, callback=None, timoeut=30):
        '''
        ...
        '''
        params = {'mode': mode}
        return self.execute_command('setInfraLedConfig', params, callback=callback, timeout=timeout)
    ...
    # *************** AV Function *******************
    def get_motion_detect_config(self, callback=None, timeout=30):
        '''
        Get motion detect config
        '''
        return self.execute_command('getMotionDetectConfig', callback=callback, timeout=timeout)

    def set_motion_detect_config(self, params, callback=None, timeout=30):
        '''
        Get motion detect config
        '''
        return self.execute_command('setMotionDetectConfig', params, callback=callback, timeout=timeout)

    def configure_motion_detection(self, params, callback=None, timeout=30):
        '''
        Get the current config and set the config without enabling or disabling
        '''
        result, current_config = self.get_motion_detect_config(timeout=timeout)
        if result != FOSCAM_SUCCESS:
            return result

        params["isEnable"] = current_config['isEnable']
        self.set_motion_detect_config(params, timeout=timeout)
        return FOSCAM_SUCCESS

    def set_motion_detection(self, enabled=1, timeout=30):
        '''
        ...
        self.set_motion_detect_config(current_config, timeout=timeout)
        return FOSCAM_SUCCESS

    def enable_motion_detection(self, timeout=30):
        '''
        Enable motion detection
        '''
        result = self.set_motion_detection(1, timeout=timeout)
        return result

    def disable_motion_detection(self, timeout=30):
        '''
        disable motion detection
        '''
        result = self.set_motion_detection(0, timeout=timeout)
        return result

    # These API calls support FI9900P devices, which use a different CGI command
    def get_motion_detect_config1(self, callback=None, timeout=30):
        '''
        Get motion detect config
        '''
        return self.execute_command('getMotionDetectConfig1', callback=callback, timeout=timeout)

    def set_motion_detect_config1(self, params, callback=None, timeout=30):
        '''
        Get motion detect config
        '''
        return self.execute_command('setMotionDetectConfig1', params, callback=callback, timeout=timeout)
